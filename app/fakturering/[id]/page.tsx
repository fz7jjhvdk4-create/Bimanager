...
// Dynamisk import för PDF-komponenten (kräver client-side rendering)
...
export default function FakturaDetailPage({
  ...
  if (!invoice) {
    return (
      <div className="text-center py-8">
        <p className="text-amber-600">Faktura hittades inte</p>
        <Link href="/fakturering" className="text-amber-700 underline mt-2">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-amber-600">{invoice.kund.namn}</p>
            </div>
          </div>
        </div>
      ...
      {/* Invoice Preview */}
      <div className="bg-white rounded-xl shadow-sm ring-1 ring-amber-100 overflow-hidden print:shadow-none print:ring-0">
        {/* Invoice Header */}
        <div className="p-8 border-b border-amber-100">
          <div className="flex justify-between">
            <div>
              <h2 className="text-3xl font-bold text-amber-900 mb-1">FAKTURA</h2>
              <p className="text-amber-600">{invoice.fakturaNummer}</p>
            </div>
        ...
        {/* Customer & Dates */}
        <div className="p-8 grid grid-cols-2 gap-8 border-b border-amber-100">
          <div>
            <h3 className="text-sm font-medium text-amber-600 mb-2">
              FAKTURERAS TILL
            </h3>
            <div className="space-y-1">
              <p className="font-semibold text-amber-900">{invoice.kund.namn}</p>
          ...
          <div className="text-right">
            <div className="space-y-2">
              <div>
                <p className="text-sm text-amber-600">Fakturadatum</p>
                <p className="font-medium text-amber-900">
                  {new Date(invoice.fakturaDatum).toLocaleDateString("sv-SE")}
                </p>
              </div>
              <div>
                <p className="text-sm text-amber-600">Förfallodatum</p>
                <p
                  className={`font-medium ${isOverdue ? "text-red-600" : "text-amber-900"}`}
                >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-amber-200">
                <th className="text-left py-3 text-sm font-medium text-amber-700">
                  Beskrivning
                </th>
                <th className="text-right py-3 text-sm font-medium text-amber-700">
                  Antal
                </th>
                <th className="text-right py-3 text-sm font-medium text-amber-700">
                  à-pris
            ...
            <tbody>
              {rader.map((rad, index) => (
                <tr key={index} className="border-b border-amber-100">
                  <td className="py-3 text-amber-900">{rad.beskrivning}</td>
        ...
        <div className="p-8 bg-amber-50 border-t border-amber-100">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
            <div>
              <p className="font-medium text-amber-800 mb-1">Avsändare</p>
              <p className="text-amber-600">{settings?.foretagsnamn || "BiManager"}</p>
              {settings?.adress && <p className="text-amber-600">{settings.adress}</p>}
              {(settings?.postnummer || settings?.ort) && (
                <p className="text-amber-600">{settings.postnummer} {settings.ort}</p>
              )}
            </div>
            <div>
              <p className="font-medium text-amber-800 mb-1">Kontakt</p>
              {settings?.telefon && <p className="text-amber-600">Tel: {settings.telefon}</p>}
              {settings?.epost && <p className="text-amber-600">{settings.epost}</p>}
            </div>
            <div>
              <p className="font-medium text-amber-800 mb-1">Betalning</p>
              {settings?.bankgiro && <p className="text-amber-600">Bankgiro: {settings.bankgiro}</p>}
              {settings?.swish && <p className="text-amber-600">Swish: {settings.swish}</p>}
              <p className="text-amber-600">Ange {invoice.fakturaNummer} vid betalning</p>
            </div>
          </div>
        </div>
      </div>
